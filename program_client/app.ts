...
async function main(feePayer: Keypair) {
    ...
    /**
     * Derive the Coin Metadata so we can retrieve it later
     */
    const [coinPub] = deriveCoinMetadataPDA(
        {
            mint: mint.publicKey,
        },
        progId,
    );
    console.info("+==== Coin Metadata Address ====+");
    console.info(coinPub.toBase58());
    ...
    await mintSendAndConfirm({
        ...
        source: "Gold",
        year: "1989",
        shortDescription: "First minted in 1989 this 1oz gold coin was minted in the New Mexico Federal Mint.",
        signers: {
            feePayer: feePayer,
            funding: feePayer,
            mint: mint,
     ...
     */
    let coin = await getCoinMetadata(coinPub);
    console.info("+==== Coin Metadata ====+");
    console.info(coin);
    console.assert(coin!.assocAccount!.toBase58(), johnDoeATA.toBase58());
     ...
     * Get the Coin Metadata
     */
    coin = await getCoinMetadata(coinPub);
    console.info("+==== Coin Metadata ====+");
    console.info(coin);
    console.assert(coin!.assocAccount!.toBase58(), janeDoeATA.toBase58());
    ...
    console.info("+==== Coin Metadata ====+");
    console.info(coin);
    console.assert(typeof coin!.assocAccount, "undefined");
}
