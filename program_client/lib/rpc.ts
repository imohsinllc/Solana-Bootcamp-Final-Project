...
export const mint = (args: MintArgs): TransactionInstruction => {
    ...
    const [coinPubkey] = pda.deriveCoinMetadataPDA({
        mint: args.mint,
    }, _programId);
...
export const getCoinMetadata = async (
...
): Promise<T.CoinMetadata | undefined> => {
    ...
    return T.decodeCoinMetadata(deserialize(T.CoinMetadataSchema, buffer.data) as Record<string, unknown>);
}
