...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("coin", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let source: String = Default::default();
		let year: String = Default::default();
		...
		let (coin_pubkey, _) = Pubkey::find_program_address(
			&[b"coin", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(coin_pubkey, false),
		...
		let data = CoinInstruction::Mint(crate::generated::instructions::MintArgs{
			source,
			year,
			short_description,
		});
