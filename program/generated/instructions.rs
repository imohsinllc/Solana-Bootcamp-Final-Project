...
impl CoinInstruction {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        let (&variant, rest) = input.split_first().ok_or(CoinError::InvalidInstruction)?;

        Ok(match variant {
			0 => Self::Mint(MintArgs::try_from_slice(rest).unwrap()),
			1 => Self::Transfer,
			2 => Self::Burn,
			_ => return Err(CoinError::InvalidInstruction.into())
        })
    }
}