...
use crate::generated::errors::CoinError;
use crate::generated::instructions::CoinInstruction;
...
impl Processor {
    pub fn process(
        ...
        let instruction = CoinInstruction::unpack(data)?;

        match instruction {
			CoinInstruction::Mint(args) => {
...
/// - short_description: [String] 
	pub fn process_mint(
		...
		let coin_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (coin_pubkey, coin_bump) = Pubkey::find_program_address(
			&[b"coin", mint_info.key.as_ref()],
			program_id,
		);

		// Security Checks
		if fee_payer_info.is_signer != true {
			return Err(CoinError::InvalidSignerPermission.into());
		}
		...
		if *coin_info.key != coin_pubkey {
			return Err(CoinError::NotExpectedAddress.into());
		}
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), coin_info.clone()],
			&[&[b"coin", mint_info.key.as_ref(), &[coin_bump]]],
		)?;


		// Security Checks
		if *fee_payer_info.owner != Pubkey::from_str("11111111111111111111111111111111").unwrap() {
			return Err(CoinError::WrongAccountOwner.into());
		}
		...
		if mint_info.data_len() != spl_token::state::Mint::LEN {
			return Err(CoinError::InvalidAccountLen.into());
		}

		if coin_info.data_len() != CoinMetadata::LEN {
			return Err(CoinError::InvalidAccountLen.into());
		}
		...
		let coin = &mut AccountPDA::new(
			&coin_info,
			try_from_slice_unchecked::<CoinMetadata>(&coin_info.data.borrow()).unwrap(),
			coin_bump,
		);
		...
		coin.data.serialize(&mut &mut coin_info.data.borrow_mut()[..])?;
